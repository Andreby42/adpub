...
public class LocationKDTree {
    ...
    public Position findNearest(final double latitude, final double longitude) {
        ...
        if (GpsUtils.distance(node.position.getLng(), node.position.getLat(),
                longitude, latitude) * 1000 > node.position.getDist()) {
            return null;
        }
        ...
        if (other != null) {
            if (current.verticalDistance(target, axis) < best.euclideanDistance(target)) {
                ...
                if (possibleBest != null && 
                		(possibleBest.euclideanDistance(target) < best.euclideanDistance(target))) {
                    best = possibleBest;
                }
            }
        }
