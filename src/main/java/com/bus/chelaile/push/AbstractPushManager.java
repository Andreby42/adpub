...
public class AbstractPushManager {
    ...
    private static ExecutorService udidTokenToOcsThreadPool = Executors.newFixedThreadPool(
            Integer.parseInt(PropertiesUtils.getValue(PropertiesName.PUBLIC.getValue(), "udidToken.toOcs.threadcount")));
    ...
    public Map<String, String> getTokenByUdidsFromOcs(List<String> udidList) {
        ...
        /*final int maxRecord =
                Integer.parseInt(PropertiesUtils.getValue(PropertiesName.PUBLIC.getValue(), "max.per.redis.request"));
        
        final int sleepMSecond =
                Integer.parseInt(PropertiesUtils.getValue(PropertiesName.PUBLIC.getValue(), "max.per.redis.sleepMSecond"));
        int count = 0;
        ...
        for (int idx = 0; idx < listSize; idx++) {
            try {
                String udid = udidList.get(idx);
                String tokenStr = CacheUtil.getTokenFromRedis(udid);
                if (tokenStr == null) {
                    continue;
                }
                resultMap.put(udidList.get(idx), tokenStr);
                logger.info("getTokenByUdids 从 redis 之中获得TOKEN：" + tokenStr);
            } catch (Exception ex) {
                logger.error("getTokenByUdids redis中查询TOKEN异常: " + ex.getMessage(), ex);
            }
