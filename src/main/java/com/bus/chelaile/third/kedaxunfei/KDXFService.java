...
public class KDXFService {
	...
	private static final Logger logger = LoggerFactory.getLogger(KDXFService.class);
	...
	// */
	public void doService(AdvParam ap, ShowType type,int displayType) throws Exception {
		ThirdAdData model = getKDXF(ap, type,displayType);
	...
	public static ThirdAdData getKDXF(AdvParam ap, ShowType type, int displayType) throws Exception {
		...
		if (ap.getS().equalsIgnoreCase("android")) {
			...
			if (type.getValue() == ShowType.OPEN_SCREEN.getValue()) {
				...
				if (displayType == 3) {
					m.setAdunitid("ACB0BED305BBA908DEA75B0036E51ECE");
				} else {
				...
				if (displayType == 3) {
					m.setAdunitid("FD95828C09A32D712082DC08D36CC15D");
				} else {
			...
			}else if (type.getValue() == ShowType.TRANSFER_ADV.getValue()) {
				if (displayType == 3) {
					m.setAdunitid("AE9A21B49A87F9B6C67EC7548FB5DE48");
				} else {
					m.setAdunitid("A7ABF4CFA257C79F064A8A162266D924");
				}
			}else if (type.getValue() == ShowType.CAR_ALL_LINE_ADV.getValue()) {
				if (displayType == 3) {
					m.setAdunitid("67842F384D42AE16958A2ADDC8080BA0");
				} else {
					m.setAdunitid("3D7818F4980FB323DAEA8B544B567B7C");
				}
			}else if (type.getValue() == ShowType.ALL_CAR_ADV.getValue()) {
				if (displayType == 3) {
					m.setAdunitid("B7880FAC93B6C405F219E54DABBAD2A3");
				} else {
					m.setAdunitid("BEAD9E183E07E296B1855E2AB3F2E6ED");
				}
			}
	...
	public static void main(String[] args) throws Exception {
		...
		ap.setUa(
				"Mozilla%2F5.0+%28Linux%3B+Android+5.1.1%3B+vivo+X7+Build%2FLMY47V%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Version%2F4.0+Chrome%2F39.0.0.0+Mobile+Safari%2F537.36");
		ap.setLng(116.403538);
		ap.setLat(39.994026);
		ap.setVendor("vivo");
		// ap.setWifissid("da");
