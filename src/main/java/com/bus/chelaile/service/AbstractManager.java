...
public abstract class AbstractManager {
	...
	protected void writeSendLog(AdvParam advParam, AdContent ad, BaseAdEntity entity) {
		AnalysisLog.info(
				...
				advParam.getS(), advParam.getV(), advParam.getLineId(), advParam.getStnName(), advParam.getNw(),
				advParam.getIp(), advParam.getDeviceType(), advParam.getLng(), advParam.getLat(), advParam.getH5User(),
				advParam.getH5Src(), entity.getProvider_id());
...
//			}
	...
	private boolean ruleCheck(Rule rule, AdvParam advParam, AdContent ad, AdPubCacheRecord cacheRecord,
		...
		if (showType == ShowType.OPEN_SCREEN
			...
			logger.info("MIUI OPEN return false, deviceType={}, canPubMIUI={}, startMode={}, udid={}",
					advParam.getDeviceType(), rule.getCanPubMIUI(), advParam.getStartMode(), advParam.getUdid());
			return false;
		}
		// 冷热启动模式测试
		...
		if (advParam.getStartMode() == 1 && rule.getMinIntervalTime() > 0) {
			if (cacheRecord != null && !cacheRecord.hasPassIntervalTime(ad.getId(), rule.getMinIntervalTime())) {
				logger.info("cannot pub OpenAd, because of minInterval time not past, ruleId={}, udid={}",
						rule.getRuleId(), advParam.getUdid());
				return false;
			}
		}
...
//		}
	 ...
	 * 应该按照优先级倒叙排序
	 */
	private static final Comparator<AdContentCacheEle> AD_CONTENT_COMPARATOR=new Comparator<AdContentCacheEle>(){@Override public int compare(AdContentCacheEle o1,AdContentCacheEle o2){if(o1==null)return-1;if(o2==null)return 1;return o2.getAds().getPriority()-o1.getAds().getPriority();}};
	...
	private List<BaseAdEntity> getEntities(AdvParam advParam, AdPubCacheRecord cacheRecord,
...
//				advParam.getStnName(), advParam.getStartMode(), advParam.getH5Src(), advParam.getWxs());
	...
	protected boolean beforeCheck(AdvParam advParam, ShowType showType) {
		...
		if (showType.getType().equals(ShowType.LINE_DETAIL)) {
			if (StringUtils.isNoneBlank(advParam.getCshow())
					&& !advParam.getCshow().equals(Constants.CSHOW_LINEDETAIL)) {
				return false;
			}
		}
